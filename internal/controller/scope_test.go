...
func TestIsResourceAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "Default Allows Unprotected Resource",
			config:   ScopeConfig{},
			res:      "pods",
			group:    "",
			expected: true,
		...
		{
			name:     "Default Denies ServiceAccount",
			config:   ScopeConfig{},
			res:      "serviceaccounts",
			group:    "",
			expected: false,
		...
		{
			name:     "Default Denies RoleBinding",
			config:   ScopeConfig{},
			res:      "rolebindings",
			group:    "rbac.authorization.k8s.io",
			expected: false,
		...
		{
			name:     "Default Denies ClusterRole",
			config:   ScopeConfig{},
			res:      "clusterroles",
			group:    "rbac.authorization.k8s.io",
			expected: false,
		...
		{
			name:     "Default Denies ClusterRoleBinding",
			config:   ScopeConfig{},
		...
		{
			name:     "Default Denies PersistentVolume",
			config:   ScopeConfig{},
			res:      "persistentvolumes",
			group:    "",
			expected: false,
		},
		{
			name:     "Default Denies StorageClass",
			config:   ScopeConfig{},
			res:      "storageclasses",
		...
		{
			name:     "Default Denies CustomResourceDefinition",
			config:   ScopeConfig{},
		...
		{
			name:     "Explicit Allow Can Include Default Denied Resource",
			config:   ScopeConfig{WatchResources: []string{"secrets"}},
			res:      "secrets",
			group:    "",
			expected: true,
		},
