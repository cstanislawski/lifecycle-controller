...
- **Only for pod-spawning resources (Deployments, StatefulSets, DaemonSets, etc.):**
  ...
  - `lifecycle.cezary.dev/restart-every`: Performs a rolling restart on a recurring, relative basis (e.g., `7d` to restart weekly). Supports `s`, `m`, `h`, and `d` (days), but the resulting duration must be at least `1m`. Zero, negative, and shorter durations are invalid. The controller emits a `Warning` Event with reason `InvalidAnnotation`, takes no action, and does not requeue itself for a permanent invalid value.
  - `lifecycle.cezary.dev/restart-cron`: Performs a rolling restart based on a standard five-field, minute-resolution cron expression (e.g., `"0 3 * * *"` for daily at 3 AM). Descriptors such as `@every` are invalid; the controller emits an `InvalidAnnotation` Warning Event and does not requeue itself for a permanent invalid value.
...
- **Restart Mechanism**
  ...
  - **State Tracking for Recurring Restarts** - For `restart-every` and `restart-cron` schedules, the controller maintains its state using a top-level `lifecycle.cezary.dev/last-restart-timestamp: "<timestamp>"` annotation on the resource. This timestamp serves as the anchor for calculating the next restart, ensuring the schedule remains stable over time.
    ...
    - **Reconciliation Logic** - On each check, the controller performs the following steps:
      ...
      - If multiple occurrences were missed, it coalesces them rather than triggering one catch-up rollout per occurrence,
      - For `restart-every`, it advances `lifecycle.cezary.dev/last-restart-timestamp` directly to the latest elapsed interval, preserving the original cadence. For `restart-cron`, it advances the anchor to the current reconciliation time; the cron expression still determines the next future occurrence.
    - **Boundary behavior** - An occurrence is due when the current time equals its scheduled time. After a coalesced restart, the controller schedules only the next future occurrence.
    - **Safety rationale** - The `1m` minimum is a workload-safety boundary, not throughput tuning. Every action changes a pod template and can start a destructive rolling update. The production-safety audit behind this rule cited the need to protect downstream systems with explicit concurrency and rate controls ([KubeCon NA 2023 talk](https://www.youtube.com/watch?v=2NjMHLACvc0&t=737s)). One minute matches the resolution of standard Kubernetes [CronJob schedules](https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/#writing-a-cronjob-spec); Kubernetes also bounds CronJob catch-up after excessive missed schedules. Mature operators commonly use minute-scale, approximate reconciliation intervals and jitter, as documented by [Flux source-controller](https://fluxcd.io/flux/components/source/gitrepositories/#interval). Queue rate limiting alone is insufficient: controller-runtime `v0.22.4` sends an explicit immediate requeue through its [rate-limited queue](https://github.com/kubernetes-sigs/controller-runtime/blob/v0.22.4/pkg/internal/controller/controller.go#L461-L487), while client-go `v0.34.3` starts per-item backoff at `5ms` and permits an overall `10` QPS with a burst of `100` ([source](https://github.com/kubernetes/client-go/blob/v0.34.3/util/workqueue/default_rate_limiters.go#L48-L55)). Validation and missed-run coalescing therefore provide the rollout safety guarantee.
